...
<body>
  <div class="borrowing-form">
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
      ...
      <div class="bookid">
        <div class="book-id">
          <div class="book-id1">
            ...
            <button class="add-sahpe-parent" type="button">
      ...
      <div class="returndate">
        <div class="returndate1">
          ...
          <div class="borrow-date">
            ...
            <button type="button" class="frame2 choose-button" id="returnDateButton">
              <img class="add-sahpe-icon" alt="" src="./public/add-sahpe.svg" />
              <div class="choose">Choose</div>
            </button>
            <input type="hidden" id="returnDateInput" name="returnDate" />
          </div>
        </div>
      </div>
      <div class="borrowdate">
        ...
        <img class="borrowdate-child" alt="" src="./public/shape.svg" />
        <button type="button" class="add-sahpe-parent choose-button" id="borrowDateButton">
          <img class="add-sahpe-icon" alt="" src="./public/add-sahpe.svg" />
          <div class="choose">Choose</div>
        </button>
        <input type="hidden" id="borrowDateInput" name="borrowDate" value="<?php echo date('Y-m-d'); ?>" />
      </div>
      <div class="libraryrules">
        ...
        <div class="rules1">
          <ul class="user-must-not-have-overdue-boo1">
            ...
            <li class="user-must-not1">There is a set duration that a user may borrow the book.</li>
            <li class="user-must-not1">Failure to Return a Circulation Book.</li>
            <li class="user-must-not1">Php 2.00 a day, exclusive of Sundays and holidays.</li>
            <li class="user-must-not1">Failure to Return a Reserve Book.</li>
            <li class="user-must-not1">Php 1.00 (first hour)</li>
            <li class="user-must-not1">Php 5.00 (Each hour after the first hour)</li>
            <li class="user-must-not1">Php 50.00 (Whole day)</li>
            <li class="user-must-not1">Withdrawal of a Reserve Book Without Reservation Permit.</li>
            <li class="user-must-not1">Php 50.00</li>
            <li class="user-must-not1">Failure to Return a General Reference Book and Other Restricted Materials Borrowed for Photocopying Purposes.</li>
            <li class="user-must-not1">Php 50.00 fine</li>
            <li class="user-must-not1">Library privileges suspended for 1 week (second offense)</li>
            <li class="user-must-not1">Loss of a Circulation Book</li>
            <li class="user-must-not1">Replace it with the same title or good photocopy or pay its current replacement value</li>
            <li>The person shall pay a fine equivalent to 50% of the cost of the book</li>
          </ul>
        </div>
  ...
  <div id="borrowingFormPopUpPopup" class="popup-overlay" style="display: none">
    <div class="borrowing-form-pop-up1">
      <div class="logo-group">
        <div class="logo11">
          <img class="logo-icon3" alt="" src="./public/logo@2x.png" />
          <div class="school-name2">Sacred Heart of Jesus Catholic School</div>
          <div class="library-system7">LIBRARY SYSTEM</div>
          <b class="system2">Learning Resource Center</b>
        </div>
        <div class="container1">
          <div class="message1">
            The book is successfully reserved for you to borrow. You may claim it anytime during office hours. Failure to claim within three days will lead to forfeiture of the reservation of the book. Scheduled return date will be 7 days from now.
          </div>
          <div class="pleasesaveyourreferencecode1">Please save your reference code</div>
          <b class="referencecode1">20238758900</b>
        </div>
        <button class="submitbutton1" id="popupsubmitButton">
          <img class="submitbutton-item" alt="" src="./public/rectangle-13.svg" />
  ...
  <script>
    $(function() {
      $("#borrowDateButton").on("click", function() {
        $("#borrowDateInput").datepicker("show");
      });

      $("#returnDateButton").on("click", function() {
        $("#returnDateInput").datepicker("show");
      });

      $("#borrowDateInput").datepicker({
        dateFormat: "yy-mm-dd",
        onSelect: function(dateText) {
          $("#borrowDateButton .choose").text(dateText);
        }
      });
      ...
      $("#returnDateInput").datepicker({
        dateFormat: "yy-mm-dd",
        onSelect: function(dateText) {
          $("#returnDateButton .choose").text(dateText);
        }
      });
      ...
      $("#borrowDateButton .choose").text(today);
    });
  </script>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $borrowDate = isset($_POST["borrowDate"]) ? sanitize_input($_POST["borrowDate"]) : "";
      ...
      $borrowQuery = "INSERT INTO borrow_records (patron_id, book_id, borrow_date, return_date) VALUES (?, ?, ?, ?)";
      $stmt = $conn->prepare($borrowQuery);
      $stmt->bind_param("ssss", $patronId, $bookId, $borrowDate, $returnDate);
